...
import { Plus, FolderOpen, Trash2, ToggleLeft, ToggleRight, Edit2, ExternalLink, Upload } from "lucide-react";
...
export default function Portfolio() {
  ...
  const load = () => {
    setLoading(true);
    api.get("/portfolio?all=1").then(setItems).catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(load, []);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (selected.id) await api.put(`/portfolio/${selected.id}`, selected);
      else await api.post("/portfolio", selected);
      load();
      setSelected(null);
    } finally {
      setSaving(false);
  ...
  const toggleActive = async (item) => {
    await api.put(`/portfolio/${item.id}`, { ...item, active: item.active ? 0 : 1 });
    load();
  };
  ...
  const handleTitleChange = (e) => {
    ...
    setSelected((prev) => ({
      ...prev,
      title,
      // Only auto-generate slug if it's a new item or slug is empty / was auto-generated
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {items.filter((i) => i.active).length} active case studies
          </p>
        </div>
        <button
          ...
          <div className="w-96 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col shrink-0 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              <p className="font-semibold text-gray-800 text-sm">
                {selected.id ? "Edit Project" : "New Project"}
              </p>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  value={selected.slug}
                  onChange={(e) =>
                    setSelected({ ...selected, slug: toSlug(e.target.value) })
                  }
            ...
            <div className="px-5 pb-5 pt-3 border-t border-gray-100 flex gap-2">
              ...
              >
                {saving ? "Saving…" : selected.id ? "Save Changes" : "Create Project"}
              </button>
              <button
                onClick={() => setSelected(null)}
